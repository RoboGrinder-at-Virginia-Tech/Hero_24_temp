...
/**
  ...
  * @retval         void
  */
static void shoot_bullet_control_absolute_42mm(void); //shoot_bullet_control_42mm
static void shoot_bullet_control_continuous_42mm(uint8_t shoot_freq);
...
fp32 temp_speed_setALL = 23; //19.0f;
...
//		PID_init(&shoot_control.trigger_motor_angle_pid, PID_POSITION, Trigger_speed_pid_42mm_outerLoop, TRIGGER_BULLET_PID_42mm_OUTER_MAX_OUT,TRIGGER_BULLET_PID_42mm_OUTER_MAX_IOUT);
		shoot_PID_init(&shoot_control.trigger_motor_angle_pid, SHOOT_PID_SEPARATED_INTEGRAL_OUT_POS, Trigger_speed_pid_42mm_outerLoop, TRIGGER_BULLET_PID_42mm_OUTER_MAX_OUT,TRIGGER_BULLET_PID_42mm_OUTER_MAX_IOUT);
    ...
    shoot_control.key_time = 0;
		...
		*/
		...
		shoot_control.local_cd_rate = get_shooter_id1_42mm_cd_rate(); //通用 数据
}
...
//------------------修改等级判断 Texas A&M 比赛使用
	 if(toe_is_error(REFEREE_TOE))
   {
	 ...
	 {
		 // TODO 动态速度变化
			shoot_control.referee_current_shooter_42mm_speed_limit = 10; //get_shooter_id1_17mm_speed_limit(); -----------------DEBUG-----------------
	 }
	 
	 /*记得添加 数据超出最大合理数值时的操作*/
	 if(shoot_control.referee_current_shooter_42mm_speed_limit > 16)
	 {
		 shoot_control.referee_current_shooter_42mm_speed_limit = 16;
	 }
	 
	 //42mm 的两档
//	 shoot_control.referee_current_shooter_42mm_speed_limit = 16;//强制使其=10 用于调试---------------------------------------------------
	 if(shoot_control.referee_current_shooter_42mm_speed_limit == 10)
	 { //12.5 - 9.8m/s
		 shoot_control.currentLIM_shoot_speed_42mm = 12.0f; //12.0f; //10.0f + 3.0f;//shoot_control.referee_current_shooter_42mm_speed_limit;//=10
		 shoot_control.predict_shoot_speed = 9.0f;// - 更合理的弹速
		 /*1) 发给ZYZ那  测出来
		   2) 发给ZYZ那  测出来
		 */
	 }
	 else if(shoot_control.referee_current_shooter_42mm_speed_limit == 16)
	 {//6-15之前的自瞄一直是按这个测试的
		 // 18- 4.5 为 RMUL 实际 16.7-17.1 - .3 m/s 单速标定 SZL
		 shoot_control.currentLIM_shoot_speed_42mm = 19.0f; //20.0f; // + 3.0f;
		 shoot_control.predict_shoot_speed = 16.0f; // shoot_control.currentLIM_shoot_speed_42mm + 3;
		 /*
	 ...
	 {//默认射速10
		 shoot_control.currentLIM_shoot_speed_42mm = 12.0f; //10.0f + 3.0f;//shoot_control.referee_current_shooter_42mm_speed_limit;//=10
		 shoot_control.predict_shoot_speed = 9.0f;// - 更合理的弹速
	 }
	 
	 //新规则弹速测试 1-20 - 新规则弹速上限只有 16m/s这一档
	 shoot_control.currentLIM_shoot_speed_42mm = (fp32)temp_speed_setALL;
	 shoot_control.predict_shoot_speed = shoot_control.currentLIM_shoot_speed_42mm;
...
//				PID_clear(&shoot_control.trigger_motor_angle_pid);
    ...
    else if (shoot_control.shoot_mode == SHOOT_BULLET)
    {
        ...
        shoot_bullet_control_absolute_42mm(); //shoot_bullet_control_42mm();
    }
...
//				trigger_motor_turn_back_17mm();
				...
				shoot_control.trigger_motor_pid.max_out = TRIGGER_BULLET_PID_MAX_OUT;//-----------------------------------------
        shoot_control.trigger_motor_pid.max_iout = TRIGGER_BULLET_PID_MAX_IOUT;
				shoot_bullet_control_continuous_42mm(4);
    }
...
//        ramp_calc(&shoot_control.fric2_ramp, -SHOOT_FRIC_PWM_ADD_VALUE);
			...
			if(shoot_control.left_fricMotor.fricW_speed < 0.1f && shoot_control.right_fricMotor.fricW_speed < 0.1f)
			{
...
//		M3508_fric_wheel_spin_control(-shoot_control.currentLeft_speed_set, shoot_control.currentRight_speed_set); //移到其他地方, 保证shoot stop时0电流
...
static void shoot_set_mode(void)
{
    ...
    if ((switch_is_up(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) && !switch_is_up(last_s) && shoot_control.shoot_mode == SHOOT_STOP))
    {
        shoot_control.shoot_mode = SHOOT_READY_FRIC;//上拨一次开启摩擦轮
			  ...
			  shoot_control.key_Q_cnt = 1; //英雄按一下是semi, 按两下是auto
    }
    ...
    if (switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) && (shoot_control.shoot_rc->key.v & SHOOT_ON_KEYBOARD) && shoot_control.shoot_mode == SHOOT_STOP)
    {
        shoot_control.shoot_mode = SHOOT_READY_FRIC; 
				shoot_control.user_fire_ctrl = user_SHOOT_SEMI; //user_SHOOT_AUTO; //42mm 去掉连发模式需要修改的地方
    }
    //处于中档， 可以使用键盘关闭摩擦轮
    else if (switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) && (shoot_control.shoot_rc->key.v & SHOOT_OFF_KEYBOARD) && shoot_control.shoot_mode != SHOOT_STOP)
    {
		...
		{
			shoot_control.user_fire_ctrl = user_SHOOT_SEMI; //英雄改成按一下 semi 两下 auto
		}
...
//		}
		...
		//10-某一天-2022修改 5-9-2023 跳过 英雄永远单发
		//连续发弹判断; 发射机构断电时, shoot_mode状态机不会被置为发射相关状态
    if(shoot_control.shoot_mode > SHOOT_READY_FRIC && shoot_control.trigger_motor_42mm_is_online)
    {
        //鼠标长按一直进入射击状态 保持连发
			  ...
			  //重要 TODO: 添加 敌我识别
				...
				{
					//(( (get_shootCommand() == 0xff) && (get_autoAimFlag() > 0) ) || (shoot_control.rc_s_time == RC_S_LONG_TIME))
...
//				/* 7-4-2023 英雄关闭连发模式: 1) user_fire_ctrl永远设置为user_SHOOT_SEMI; 
//					2) 当shoot_control.shoot_mode == SHOOT_CONTINUE_BULLET时, 设置为SHOOT_BULLET - 这样耦合最低*/
//				if(shoot_control.shoot_mode == SHOOT_CONTINUE_BULLET)
//				{
//					shoot_control.shoot_mode = SHOOT_READY_BULLET; //不执行连发
//				}
    }

		//以下开始热量环
		shoot_heat_update_calculate(&shoot_control); //未使用
		
    //42mm ref热量限制 - 42就用裁判系统的就OK了
...
//利用了角度环PID的 防堵转函数
void trigger_motor_turn_back_42mm_absolute_angle() //未使用
{
		if( shoot_control.block_time < BLOCK_TIME)
    {//未发生堵转
        //shoot_control.speed_set = shoot_control.trigger_speed_set;
...
/**
  ...
  */
//42mm 摩擦轮 单发任务执行一次
static void shoot_bullet_control_absolute_42mm(void)
{
    //每次拨动 120度 的角度
...
uint32_t shoot_heat_update_calculate(shoot_control_t* shoot_heat)
{
	if(!toe_is_error(REFEREE_TOE))
  {
		 ...
		 shoot_heat->local_cd_rate = get_shooter_id1_42mm_cd_rate();
  }
	else
	{
...
//		 get_shooter_id1_17mm_heat_limit_and_heat(&shoot_heat->heat_limit, &shoot_heat->heat);
	...
	if( get_para_hz_time_freq_signal_HAL(10) )
	{
		...
		if(shoot_control.trigger_motor_42mm_is_online)
		{ //发射机构未断电
#if TRIG_MOTOR_TURN
...
//		shoot_heat->rt_odom_local_heat = (fp32)(shoot_heat->rt_odom_angle - shoot_heat->last_rt_odom_angle) / ((fp32)RAD_ANGLE_FOR_EACH_HOLE_HEAT_CALC) * ONE17mm_BULLET_HEAT_AMOUNT; //不这样算
			...
			shoot_heat->rt_odom_local_heat[0] += (fp32)abs( ((int32_t)shoot_heat->rt_odom_total_bullets_fired) - ((int32_t)shoot_heat->rt_odom_calculated_bullets_fired) ) * (fp32)ONE42mm_BULLET_HEAT_AMOUNT;
